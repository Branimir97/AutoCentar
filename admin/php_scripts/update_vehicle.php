   ...
   $phpFileUploadErrors = array(
    ...
    if(isset($_POST['submit'])){
        ...
        $query = "UPDATE vehicles SET 
            type=:type, status=:status, title=:title, mark=:mark, 
            model=:model, manufacture_year=:manufacture_year, model_year=:model_year, 
            kilometers=:kilometers, motor_power=:motor_power, transmission=:transmission, 
            gears=:gears, motor_capacity=:motor_capacity, consumption=:consumption, additional_equipment=:additional_equipment, 
        ...
        if(!empty($_FILES['images']['name'][0]))  {
            ...
            if(count($image_urls)>=3 && count($image_urls) <=10) {
                $update_images = "UPDATE vehicles SET images=:images WHERE id = '{$id}'";
                $stmt2 = $pdo->prepare($update_images);
                $stmt2->execute(array('images' => json_encode($image_urls, JSON_UNESCAPED_SLASHES)));
